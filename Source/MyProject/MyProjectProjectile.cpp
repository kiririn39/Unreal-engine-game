...
void AMyProjectProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	AADestructionTarget* target = Cast<AADestructionTarget>(OtherActor);
	// check if this object collided with target class and if it is, invoke target's destruction logic
	if (target != nullptr)
